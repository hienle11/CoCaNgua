...
public class ComputerPlayer extends Player
{
    ...
    public Cell makeDecision(HashMap<Cell, Integer[]> possibleMoves, int[] diceValue)
    {
        ...
        for(Cell cell: possibleCellSet)
        {
            ...
            {
                if (cell.getId().contains("Home") && movePriority <= 4)       // check if the Chess is arriving Home
                {
                    if (!pickedCell.getId().contains("Home"))       // if the pickedCell is not a HomeArrivalMove
                        itIsABetterMove = true;
                    else if (pickedStep < diceValue[diceIndex])     // if it is, compare which move leads the chess to higher position
                        itIsABetterMove = true;
                }
                else if (cell.getId().contains("Spawn") && movePriority <=3)  // check if the Chess is spawning
                {
                    itIsABetterMove = true;
                }
